...
function delete_sns_subscriptions() {
  for account in "development" "staging" "production"; do
    ...
    for arn in $(${aws_cross_account} sns list-subscriptions --query 'Subscriptions[?Endpoint==`'${user}'@pocketmath.com`].SubscriptionArn' | awk -F '"' '/arn/{ print $2 }'); do
      if [ ! -z "$arn" ]; then
